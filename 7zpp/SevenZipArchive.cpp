...
const std::vector<std::wstring>& SevenZipArchive::GetItemsNames()
{
	if (!m_ReadMetadata)
	{
		m_ReadMetadata = ReadInArchiveMetadata();
...
{
	return UsefulFunctions::GetNumberOfItems(*m_library, m_archivePath, m_compressionFormat, m_password, m_numberofitems);
}

bool SevenZipArchive::pri_GetItemsNames()
{
	return UsefulFunctions::GetItemsNames(*m_library, m_archivePath, m_compressionFormat, m_password,
